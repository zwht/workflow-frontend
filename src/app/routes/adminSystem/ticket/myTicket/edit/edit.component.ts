...
export class MyTicketEditComponent implements OnInit {
  ...
  dealersChange(e) {
    this.dealersList.forEach(item => {
      if (item.id === e) {
        this.ticketObj.marketId = item.inviteId
        this.ticketObj.brandId = item.brandId
        this.ticketObj.address = item.address
      }
    })
  }
  setTrSum() {
  ...
  getUser() {
    ...
    this.http
      .post(`./v1/brand/list?pageNum=1&pageSize=10`, {})
      .subscribe((res: ResponsePageVo) => {
        this.brandList = res.response.data
      })
  }
