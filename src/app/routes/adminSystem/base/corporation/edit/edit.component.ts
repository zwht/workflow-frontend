...
export class CorporationEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.id) {
      this.title = '编辑';
      this.http.get(`/cfmy/corporation/getById?id=${this.id}`)
  ...
  save(value: any) {
    ...
    if (this.id) {
      this.http.post(`/cfmy/corporation/update`, value).subscribe(res => {
        this.msgSrv.success('修改成功');
        this.back();
      });
    } else {
      this.http.post(`/cfmy/corporation/add`, value).subscribe(res => {
